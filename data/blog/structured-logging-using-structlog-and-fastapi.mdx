...
A minimal `app/__init__.py` I use, looks like the following:
...
We can now create a `app/main.py` containing the FastAPI initialiser:
...
Here we see the first usage the abovementioned [context variables](https://www.structlog.org/en/stable/contextvars.html#context-variables). This middleware is the point where we will be binding variables that will be included in every subsequent logging call of our web request.
...
We can now make use of the `get_item` function defined in the "database layer" and create an endpoind that accepts an ID as a path parameter. If the item exists in our database, we return it as a JSON with a status code 200, otherwise we return a message "Item not found" with a status code 404.
...
Notice how our logs associated with the request, contain the same `request_id` (UUID), `path`, `method` and `client_host` generated from the FastAPI middleware we defined above, even if they are generated from different files within the app. The last log contains the `status_code` added after our request is processed and we know the its value from the response object.
...
In order to avoid that, I always add a check in the abovementioned FastAPI middleware that tests if the requested path is `/healtcheck` and exludes it from producing logs. The rest of the middleware functionality remains as is:
...
## TODO conclusion, formatting and sample code commit on GitHub
